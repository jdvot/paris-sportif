...
class Team(Base):
    ...
    # Stats cache
    avg_goals_scored_home: Mapped[Optional[Decimal]] = mapped_column(Numeric(4, 2), nullable=True)
    avg_goals_scored_away: Mapped[Optional[Decimal]] = mapped_column(Numeric(4, 2), nullable=True)
    avg_goals_conceded_home: Mapped[Optional[Decimal]] = mapped_column(Numeric(4, 2), nullable=True)
    avg_goals_conceded_away: Mapped[Optional[Decimal]] = mapped_column(Numeric(4, 2), nullable=True)
    avg_xg_for: Mapped[Optional[Decimal]] = mapped_column(Numeric(4, 2), nullable=True)
    avg_xg_against: Mapped[Optional[Decimal]] = mapped_column(Numeric(4, 2), nullable=True)
...
class Competition(Base):
    ...
    emblem_url: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
...
class Match(Base):
    ...
    home_score_ht: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # Half-time
    away_score_ht: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)

    # xG data (if available)
    home_xg: Mapped[Optional[Decimal]] = mapped_column(Numeric(4, 2), nullable=True)
    away_xg: Mapped[Optional[Decimal]] = mapped_column(Numeric(4, 2), nullable=True)

    # Odds (from bookmakers, for value calculation)
    odds_home: Mapped[Optional[Decimal]] = mapped_column(Numeric(5, 2), nullable=True)
    odds_draw: Mapped[Optional[Decimal]] = mapped_column(Numeric(5, 2), nullable=True)
    odds_away: Mapped[Optional[Decimal]] = mapped_column(Numeric(5, 2), nullable=True)
...
class Prediction(Base):
    ...
    value_score: Mapped[Optional[Decimal]] = mapped_column(Numeric(6, 4), nullable=True)

    # Daily pick selection
    is_daily_pick: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
    pick_rank: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)  # 1-5 for daily picks
    pick_score: Mapped[Optional[Decimal]] = mapped_column(Numeric(6, 4), nullable=True)

    # LLM-generated explanation
    explanation: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    key_factors: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON array
    ...
    llm_adjustments: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    ...
    result: Mapped[Optional["PredictionResult"]] = relationship("PredictionResult", back_populates="prediction", uselist=False)

    # Indexes
    __table_args__ = (
        Index("ix_predictions_daily_pick", "is_daily_pick", "created_at"),
    )
...
class PredictionResult(Base):
    ...
    value_captured: Mapped[Optional[Decimal]] = mapped_column(Numeric(6, 2), nullable=True)
...
class NewsItem(Base):
    ...
    # Related entities
    team_ids: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON array
    player_names: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON array

    # Analysis results
    is_injury_news: Mapped[bool] = mapped_column(Boolean, default=False)
    sentiment_score: Mapped[Optional[Decimal]] = mapped_column(Numeric(4, 3), nullable=True)  # -1 to 1
    impact_score: Mapped[Optional[Decimal]] = mapped_column(Numeric(4, 3), nullable=True)  # 0 to 1
    llm_analysis: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON
