...
class PredictionService:
    ...
    async def get_prediction(match_id: int) -> dict[str, Any] | None:
        ...
        async with get_uow() as uow:
            ...
            # Parse news_sources from DB
            news_sources_parsed = None
            if pred.news_sources:
                try:
                    news_sources_parsed = json.loads(pred.news_sources)
                except (json.JSONDecodeError, TypeError):
                    news_sources_parsed = None
            ...
            return {
                ...
                "match_context_summary": pred.match_context_summary,
                "news_sources": news_sources_parsed,
                "created_at": (
    ...
    async def save_prediction_from_api(prediction_data: dict[str, Any]) -> bool:
        ...
        try:
            async with get_uow() as uow:
                ...
                # Prepare match_context_summary and news_sources
                match_context_summary = prediction_data.get("match_context_summary")
                news_sources = prediction_data.get("news_sources")
                ...
                await uow.predictions.upsert(
                    ...
                    key_factors=json.dumps(key_factors) if key_factors is not None else None,
                    ...
                    match_context_summary=match_context_summary if match_context_summary else None,
                    news_sources=(json.dumps(news_sources) if news_sources is not None else None),
                )
    ...
    async def generate_daily_picks() -> list[dict[str, Any]]:
        """Return today's daily picks from pre-computed predictions in DB.

        All predictions are pre-generated by data_prefill_service.
        This method only reads existing daily picks — no live generation.
        """
        today = date.today()
        picks = await PredictionService.get_daily_picks(today)
        logger.info(f"Retrieved {len(picks)} daily picks from DB for {today}")
        return picks
