/**
 * Generated by orval v8.2.0 🍺
 * Do not edit manually.
 * WinRate AI API
 * API de predictions de paris sportifs sur le football europeen
 * OpenAPI spec version: 0.1.0
 */
import type { SrcApiRoutesMatchesDataSourceInfoSource } from './srcApiRoutesMatchesDataSourceInfoSource';

/**
 * Information about data source and any warnings (Beta feature).

Distinguishes between:
- external_api_limited: football-data.org rate limit (10 req/min free tier)
- app_rate_limited: our API rate limit for user protection
 */
export interface SrcApiRoutesMatchesDataSourceInfo {
  source?: SrcApiRoutesMatchesDataSourceInfoSource;
  is_fallback?: boolean;
  warning?: string | null;
  warning_code?: string | null;
  details?: string | null;
  retry_after_seconds?: number | null;
}
